...
export default function ChatComponent() {
  ...
  return (
    <div
      ...
      className="grid grid-cols-1 md:grid-cols-3 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl shadow-sm overflow-hidden select-none relative transition-colors duration-200"
    >
      {/* Left Column Navigation Sidebar */}
      <div className="border-r border-slate-200 dark:border-slate-800 flex flex-col h-full bg-slate-50/20 dark:bg-slate-900/50 overflow-hidden">
        {/* Top Toggle Switch & Add Actions */}
        <div className="p-4 border-b border-slate-200 dark:border-slate-800 space-y-3">
          <div className="flex items-center justify-between gap-2">
            {/* Filter Toggle Swapper */}
            <div className="flex bg-slate-100 dark:bg-slate-800 p-0.5 rounded-xl border border-slate-200/60 dark:border-slate-700/60 w-fit">
              <button
                onClick={() => {
                  setChatType('direct');
                  setSearchQuery('');
          ...
          <div className="relative">
            ...
            <input
              ...
              className="w-full pl-8 pr-4 py-1.5 border border-slate-200 dark:border-slate-700 rounded-xl text-[11px] outline-hidden focus:border-indigo-500 bg-white dark:bg-slate-800 text-slate-800 dark:text-slate-100 placeholder-slate-400 dark:placeholder-slate-500 transition"
            />
          </div>
        </div>
        ...
        <div className="flex-1 overflow-y-auto p-2 space-y-1">
          {isLoadingSidebar ? (
                ...
                return (
                  <button
                    ...
                    className={`w-full flex items-center gap-3 p-3 rounded-2xl text-left cursor-pointer transition-all duration-150 ${
                      isSelected
                        ? 'bg-indigo-50 dark:bg-indigo-950/60 text-indigo-600 dark:text-indigo-400 border border-indigo-100/50 dark:border-indigo-800/50 shadow-xs'
                        : 'hover:bg-slate-50 dark:hover:bg-slate-800/60 border border-transparent text-slate-600 dark:text-slate-300'
                    }`}
                  >
                    <div className="relative shrink-0">
                      <div className="h-9 w-9 rounded-xl bg-indigo-50 dark:bg-indigo-950 text-indigo-700 dark:text-indigo-300 flex items-center justify-center font-extrabold text-[11px] uppercase">
                        {getUserInitials(displayName)}
                      </div>
                      {isOnline && (
                        <span className="absolute -bottom-0.5 -right-0.5 h-2.5 w-2.5 bg-emerald-500 border-2 border-white dark:border-slate-900 rounded-full"></span>
                      )}
                    </div>
                    <div className="flex-1 min-w-0">
                      <div className="flex justify-between items-center">
                        <h4 className="text-[11px] font-bold text-slate-700 dark:text-slate-200 truncate capitalize">
                          {displayName}
                        </h4>
            ...
            filteredGroups.length === 0 ? (
              <div className="text-center p-8 space-y-1">
                <Users className="mx-auto text-slate-300 dark:text-slate-600" size={20} />
                <p className="text-[11px] text-slate-400 dark:text-slate-500 font-semibold">No group chats available</p>
              </div>
            ) : (
              filteredGroups.map((group) => {
                ...
                return (
                  ...
                  >
                    <div className="h-9 w-9 rounded-xl bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300 flex items-center justify-center font-extrabold text-[11px] uppercase shrink-0">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <span className="text-[8px] text-slate-400 dark:text-slate-500 font-bold uppercase tracking-wider block mt-0.5">
      ...
      <div className="md:col-span-2 flex flex-col justify-between h-full bg-slate-50/10 dark:bg-slate-950/40 overflow-hidden">
        {((chatType === 'direct' && activeRoom) || (chatType === 'group' && activeGroup)) ? (
          <>
            {/* Header Area */}
            <div className="px-6 py-3.5 border-b border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 flex items-center justify-between shrink-0">
              <div className="flex items-center gap-3">
                {chatType === 'direct' ? (
                  <>
                    ...
                    <div>
                      <h3 className="text-[12px] font-bold text-slate-800 dark:text-slate-100 capitalize leading-tight">
                  ...
                  <>
                    ...
                    <div>
                      ...
                      <p className="text-[9px] text-slate-400 dark:text-slate-500 font-bold uppercase tracking-wider mt-0.5">
                        {activeGroup.group_type === 'mentor_group' ? 'Staff Room' : 'Batch Cohort'}
                      </p>
                    </div>
                  </>
                )}
            ...
            <div className="flex-1 overflow-y-auto p-6 space-y-4">
              {isLoadingMessages ? (
                ...
                <div className="flex flex-col items-center justify-center h-full text-slate-400 dark:text-slate-500 space-y-1">
                  <MessageSquare size={28} className="stroke-1 text-slate-300 dark:text-slate-600" />
                  <p className="text-[11px] font-bold text-slate-500 dark:text-slate-400">Welcome to the conversation!</p>
                  <p className="text-[10px] text-slate-400 dark:text-slate-500">Type a message below to start chatting.</p>
                </div>
              ) : (
                messages.map((msg) => {
                        ...
                        <div
                          className={`p-3.5 rounded-2xl text-[11px] leading-relaxed font-semibold shadow-xs border ${
                            isMe
                              ...
                              : 'bg-white dark:bg-slate-800 text-slate-800 dark:text-slate-100 border-slate-200/70 dark:border-slate-700/70 rounded-tl-none'
                          }`}
                        >
                          {msg.message}
                        </div>
                        <span className={`text-[8px] text-slate-400 dark:text-slate-500 font-bold px-1 ${isMe ? 'text-right' : 'text-left'}`}>
              ...
              {chatType === 'direct' && typingUser && (
                <div className="flex justify-start items-center gap-2">
                  <div className="flex items-center gap-1.5 p-3.5 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-2xl rounded-tl-none shadow-xs">
                  ...
                  <span className="text-[8px] text-slate-400 dark:text-slate-500 font-bold capitalize">{typingUser.split('@')[0]} is typing...</span>
                </div>
              )}
            ...
            {/* Input action bar */}
            <form onSubmit={handleSendMessage} className="p-3.5 border-t border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 flex gap-3 items-center shrink-0">
              <input
                type="text"
                value={messageInput}
                ...
                className="flex-1 px-4 py-2.5 rounded-xl border border-slate-200 dark:border-slate-700 text-[11px] outline-hidden focus:border-indigo-500 bg-slate-50/50 dark:bg-slate-800/50 text-slate-800 dark:text-slate-100 placeholder-slate-400 dark:placeholder-slate-500 hover:bg-slate-50 dark:hover:bg-slate-800 focus:bg-white dark:focus:bg-slate-800 transition"
              />
              <button
                type="submit"
                disabled={!messageInput.trim()}
                className="p-2.5 bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl transition cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed shadow-md shadow-indigo-100 dark:shadow-none flex items-center justify-center shrink-0"
              >
                <Send size={13} />
              </button>
            </form>
          ...
          <div className="flex-1 flex flex-col items-center justify-center text-slate-400 dark:text-slate-500 space-y-3 p-8 text-center">
            <div className="h-14 w-14 bg-indigo-50 dark:bg-indigo-950/60 text-indigo-600 dark:text-indigo-400 rounded-2xl flex items-center justify-center shadow-inner">
              <MessageSquare size={24} />
            </div>
            <div className="max-w-xs space-y-1">
              <h3 className="text-[12px] font-bold text-slate-800 dark:text-slate-200">No Chat Selected</h3>
      ...
      {isModalOpen && (
        <div className="fixed inset-0 bg-slate-900/40 dark:bg-slate-950/60 backdrop-blur-xs flex items-center justify-center z-50 animate-fade-in">
          <div className="bg-white dark:bg-slate-900 rounded-3xl p-6 shadow-xl w-full max-w-sm border border-slate-100 dark:border-slate-800 mx-4 space-y-4">
            <div className="flex items-center justify-between">
              <h3 className="text-xs font-bold text-slate-800 dark:text-slate-100">Create Staff or Student Cohort</h3>
              <button
                onClick={() => {
                  setIsModalOpen(false);
                  setNewGroupName('');
            ...
            <form onSubmit={handleCreateGroup} className="space-y-4">
              <div className="space-y-1">
                <label className="block text-[9px] font-bold text-slate-500 dark:text-slate-400 uppercase">Group / Batch Name</label>
                <input
                  type="text"
                  required
                  ...
                  className="w-full px-3.5 py-2 border border-slate-200 dark:border-slate-700 rounded-xl text-[11px] outline-hidden focus:border-indigo-500 bg-white dark:bg-slate-800 text-slate-800 dark:text-slate-100 placeholder-slate-400 dark:placeholder-slate-500"
                />
              </div>

              <div className="space-y-1">
                <label className="block text-[9px] font-bold text-slate-500 dark:text-slate-400 uppercase">Group Type</label>
                <select
                  value={newGroupType}
                  onChange={(e) => setNewGroupType(e.target.value)}
                  className="w-full px-3 py-2 border border-slate-200 dark:border-slate-700 rounded-xl text-[11px] bg-white dark:bg-slate-800 text-slate-800 dark:text-slate-100 outline-hidden focus:border-indigo-500"
                >
              ...
              <div className="flex justify-end gap-3 pt-2">
                <button
                  ...
                  className="px-4 py-2 border border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-800 rounded-xl text-[11px] font-bold cursor-pointer transition"
                >
                  Cancel
                </button>
                <button
