...
import {
    ...
    ShieldCheck, ShieldOff, Mail, Calendar
} from "lucide-react";
...
export default function AdminMentors() {
    ...
    const fetchMentors = async (search = "") => {
        ...
        setError("");
        try {
            const response = await api.get("/api/adminpanel/mentors/", {
                params: search ? { search } : {},
            });
            setMentors(response.data || []);
        } catch (err) {
            console.error("Error fetching mentors:", err);
            setError("Failed to load mentors from backend API.");
        } finally {
            setIsLoading(false);
        }
    };
    ...
    const openMentorDetail = async (mentorId) => {
        setSelectedMentorId(mentorId);
        setIsDetailLoading(true);
        setMentorDetail(null);
        try {
            const response = await api.get(`/api/adminpanel/mentors/${mentorId}/`);
    ...
    const handleToggleStatus = async (mentorId) => {
        setActionLoadingId(mentorId);
        setActionError("");
        try {
            const response = await api.patch(`/api/adminpanel/mentors/${mentorId}/update/`);
            const newActiveState = response.data?.is_active;

            setMentors((prev) =>
                prev.map((m) => (m.id === mentorId ? { ...m, is_active: newActiveState } : m))
            );

            if (mentorDetail?.id === mentorId) {
                setMentorDetail((prev) => ({ ...prev, is_active: newActiveState }));
            }
        } catch (err) {
            console.error("Error toggling mentor status:", err);
            setActionError("Failed to update mentor status.");
        } finally {
    ...
    const handleDeleteMentor = async () => {
        setIsDeleting(true);
        try {
            await api.delete(`/api/adminpanel/mentors/${deletingMentor.id}/delete/`);
            setMentors((prev) => prev.filter((m) => m.id !== deletingMentor.id));
            if (selectedMentorId === deletingMentor.id) closeMentorDetail();
            ...
            <DashboardLayout role="admin" user={user}>
                <div className="flex items-center justify-center p-12 min-h-[300px]">
                    <div className="flex flex-col items-center gap-3">
                        ...
                        <p className="text-xs font-semibold text-slate-500">Loading mentor directory...</p>
                    </div>
                </div>
            </DashboardLayout>
        );
            ...
            <div className="space-y-6 animate-fade-in">
                {error && (
                    <div className="bg-rose-50 text-rose-600 border border-rose-100 px-4 py-3 rounded-xl text-xs font-semibold flex justify-between items-center">
                        ...
                        <button onClick={() => fetchMentors(searchTerm)} className="underline uppercase tracking-wider text-[10px] font-bold">Retry</button>
                    </div>
                )}

                <div className="flex flex-col sm:flex-row justify-between sm:items-center gap-4">
                    <div>
                        <h2 className="text-2xl font-extrabold text-slate-800 dark:text-slate-100">Mentor Management</h2>
                        <p className="text-sm text-slate-500 dark:text-slate-400 font-medium">Review, activate/suspend, or remove mentor accounts</p>
                    </div>
                        ...
                        <table className="w-full text-left border-collapse">
                            ...
                            <tbody className="divide-y divide-slate-100 dark:divide-slate-800 text-sm font-semibold text-slate-700 dark:text-slate-300">
                                {isLoading ? (
                                    ...
                                    <tr>
                                        <td colSpan="7" className="py-6 text-center text-slate-400 dark:text-slate-500 font-medium">No mentors found.</td>
                                    </tr>
                                ) : (
                                    mentors.map((m) => (
                                        <tr key={m.id}>
                                            <td className="py-4 text-slate-800 dark:text-slate-200">{m.username}</td>
                                            <td className="py-4 text-xs font-medium text-slate-500 dark:text-slate-400">{m.email}</td>
                                            <td className="py-4">
                                                <span className={`px-2 py-0.5 text-[9px] font-bold rounded uppercase
                           ${m.is_verified ? "bg-emerald-50 dark:bg-emerald-950/60 text-emerald-600 dark:text-emerald-400" : "bg-amber-50 dark:bg-amber-950/60 text-amber-600 dark:text-amber-400"}`}>
                                                    {m.is_verified ? "Yes" : "No"}
                                                </span>
                                            </td>
                                            <td className="py-4">
                                                <span className={`px-2 py-0.5 text-[9px] font-bold rounded uppercase
                           ${m.mfa_enabled ? "bg-indigo-50 dark:bg-indigo-950/60 text-indigo-600 dark:text-indigo-400" : "bg-slate-50 dark:bg-slate-800 text-slate-400 dark:text-slate-500"}`}>
                                                    {m.mfa_enabled ? "On" : "Off"}
                                                </span>
                                            </td>
                                            <td className="py-4">
                                                <span className={`px-2.5 py-1 text-[10px] font-bold rounded-full border
                           ${m.is_active ? "bg-emerald-50 dark:bg-emerald-950/60 text-emerald-600 dark:text-emerald-400 border-emerald-100 dark:border-emerald-900/60" : "bg-rose-50 dark:bg-rose-950/60 text-rose-500 dark:text-rose-400 border-rose-100 dark:border-rose-900/60"}`}>
                             ...
                             ${m.is_active ? "hover:bg-amber-50 dark:hover:bg-amber-950/60 text-amber-600 dark:text-amber-400" : "hover:bg-emerald-50 dark:hover:bg-emerald-950/60 text-emerald-600 dark:text-emerald-400"}`}
                                                    title={m.is_active ? "Suspend Mentor" : "Activate Mentor"}
                                                >
                ...
                <div className="fixed inset-0 bg-slate-900/40 dark:bg-slate-950/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
                    <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl w-full max-w-md shadow-xl">
                        <div className="border-b border-slate-100 dark:border-slate-800 px-6 py-4 flex justify-between items-center">
                            <h3 className="text-lg font-bold text-slate-800 dark:text-slate-100">Mentor Detail</h3>
                            <button onClick={closeMentorDetail} className="text-slate-400 hover:text-slate-600 dark:hover:text-slate-200 cursor-pointer">
                                <X size={20} />
                            </button>
                        </div>
                            ...
                            {isDetailLoading ? (
                                ...
                                <>
                                    <div className="flex items-center justify-between">
                                        <div>
                                            <h4 className="text-lg font-black text-slate-800 dark:text-slate-100">{mentorDetail.username}</h4>
                                            <p className="text-xs text-slate-400 dark:text-slate-500 font-medium flex items-center gap-1.5 mt-1">
                                                <Mail size={12} /> {mentorDetail.email}
                                            </p>
                                        </div>
                                        <span className={`px-2.5 py-1 text-[10px] font-bold rounded-full border
                       ${mentorDetail.is_active ? "bg-emerald-50 dark:bg-emerald-950/60 text-emerald-600 dark:text-emerald-400 border-emerald-100 dark:border-emerald-900/60" : "bg-rose-50 dark:bg-rose-950/60 text-rose-500 dark:text-rose-400 border-rose-100 dark:border-rose-900/60"}`}>
                                            {mentorDetail.is_active ? "Active" : "Suspended"}
                                        </span>
                                    </div>
                                    ...
                                    <div className="grid grid-cols-2 gap-3">
                                        <div className="bg-slate-50 dark:bg-slate-800 rounded-xl p-3">
                                            ...
                                            <span className="flex items-center gap-1.5 text-sm font-bold text-slate-700 dark:text-slate-200 mt-1">
                                                {mentorDetail.is_verified ? <ShieldCheck size={14} className="text-emerald-500" /> : <ShieldOff size={14} className="text-amber-500" />}
                                                {mentorDetail.is_verified ? "Yes" : "No"}
                                            </span>
                                        </div>
                                        <div className="bg-slate-50 dark:bg-slate-800 rounded-xl p-3">
                                            <span className="text-[9px] font-bold text-slate-400 dark:text-slate-500 uppercase tracking-wider block">MFA Enabled</span>
                                            <span className="flex items-center gap-1.5 text-sm font-bold text-slate-700 dark:text-slate-200 mt-1">
                                                {mentorDetail.mfa_enabled ? <ShieldCheck size={14} className="text-indigo-500" /> : <ShieldOff size={14} className="text-slate-400" />}
                                                {mentorDetail.mfa_enabled ? "On" : "Off"}
                                            </span>
                                        </div>
                                    </div>
                                    ...
                                    <div className="flex gap-4 text-xs text-slate-500 dark:text-slate-400 font-medium border-t border-slate-50 dark:border-slate-800 pt-4">
                                        <span className="flex items-center gap-1.5">
                                            <Calendar size={13} /> Joined {new Date(mentorDetail.created_at).toLocaleDateString()}
                                        </span>
                                        {mentorDetail.updated_at && (
                                            <span className="flex items-center gap-1.5">
                                                Updated {new Date(mentorDetail.updated_at).toLocaleDateString()}
                                            </span>
                                        )}
                                    </div>
                                    ...
                                    <div className="flex gap-3 pt-2">
                                        <button
                                            onClick={() => handleToggleStatus(mentorDetail.id)}
                                            disabled={actionLoadingId === mentorDetail.id}
                         ...
                         ${mentorDetail.is_active
                                        ...
                                        >
                                            {mentorDetail.is_active ? (<><Ban size={14} /> Suspend</>) : (<><CheckCircle2 size={14} /> Activate</>)}
                                        </button>
                                        <button
                                            onClick={() => setDeletingMentor(mentorDetail)}
                                ...
                                <p className="text-xs text-slate-400 dark:text-slate-500 text-center py-8">Failed to load mentor details.</p>
                            )}
                        </div>
                    </div>
                </div>
            ...
            {deletingMentor && (
                <div className="fixed inset-0 bg-slate-900/40 dark:bg-slate-950/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
                    <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl p-6 w-full max-w-sm shadow-xl space-y-4">
                        <h3 className="text-base font-bold text-slate-800 dark:text-slate-100">Delete Mentor?</h3>
                        <p className="text-xs text-slate-500 dark:text-slate-400">
                            This will permanently remove <span className="font-bold text-slate-700 dark:text-slate-200">{deletingMentor.email}</span>. Their courses may be affected. This action cannot be undone.
                        </p>
                        <div className="flex gap-3 pt-2">
                            <button
...
// dashboard/admin/AdminMentors.jsx